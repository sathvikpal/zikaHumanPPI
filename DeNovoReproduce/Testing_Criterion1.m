...
Threshold_T=0:0.1:0.9; % Dissimilarity threshold
for Cu=1:length(Threshold_T)
    
    %% STEP 1: Loop Paramters 
    CrntCut=Threshold_T(Cu);
  FeatureFile=['Features_C1_T' num2str(CrntCut) '.mat'];
  ResultFile=['Result_C1_Prob_MOD_T' num2str(CrntCut) '.mat'];
...
-q : quiet mode (no outputs)
        ...
        parfor s=1:SetCount
            ...
            % leave-one-out 
            Included=setdiff([1:SetCount],s); %#ok<*NBRAK>
            Crnt_Train=[];
            Crnt_Label=[];
            for si=1:SetCount-1
                Crnt_Train=[Crnt_Train;TrainingSp{Included(si),1}];
                Crnt_Label=[Crnt_Label;Training_LabelSp{Included(si),1}]; %#ok<*PFBNS>
            end
        ...
        Averages(7)=sum(SVratio.*Ts_Size)/sum(Ts_Size); % Weighted Average SVratio
    ...
    %% STEP OUTPUT: Save 
    save(ResultFile,'ConfideneceMat','FeatureFile','ParVect_cell','Averages_cell','Stat_Ts_cell','Stat_Tr_cell','endtime_cell')
end
